...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($user_otp == $_SESSION['temp_user']['code']) {
        try {
            ...
            // --- UPDATED INSERT QUERY TO MATCH NEW COLUMNS ---
            $stmt = $pdo->prepare("INSERT INTO users (first_name, last_name, email, password, phone_number) VALUES (?, ?, ?, ?, ?)");
            $stmt->execute([
                $_SESSION['temp_user']['first_name'], // Matches the key in signup.php
                $_SESSION['temp_user']['last_name'],  // Matches the key in signup.php
            ...
            // Combining names for the display session
            $_SESSION['user_name'] = $_SESSION['temp_user']['first_name'] . " " . $_SESSION['temp_user']['last_name'];
